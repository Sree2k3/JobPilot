...
def test_load_index_single_profile():
    """Single profile with email in combined dict is indexed."""
    with tempfile.TemporaryDirectory() as tmp:
        profile = {
            "form_data": {"email": "alice@example.com", "full_name": "Alice"},
            "combined": {"email": "alice@example.com", "skills": []},
        }
        fpath = Path(tmp) / "profile_Alice_0.json"
        fpath.write_text(json.dumps(profile), encoding="utf-8")

        index = _load_existing_profile_index(Path(tmp))
        assert index == {"alice@example.com": "profile_Alice_0.json"}
...
def test_load_index_form_data_fallback():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        profile = {
            ...
            "combined": {"skills": []},  # no email key
        }
        fpath = Path(tmp) / "profile_Bob_0.json"
        fpath.write_text(json.dumps(profile), encoding="utf-8")

        index = _load_existing_profile_index(Path(tmp))
        assert index == {"bob@test.com": "profile_Bob_0.json"}


def test_load_index_case_insensitive():
    """Email keys are stored lowercase."""
    with tempfile.TemporaryDirectory() as tmp:
        profile = {
            "form_data": {"email": "Alice.Wonderland@Example.Com"},
            "combined": {},
        }
        fpath = Path(tmp) / "profile_Alice_0.json"
        fpath.write_text(json.dumps(profile), encoding="utf-8")
        ...
        index = _load_existing_profile_index(Path(tmp))
        assert "alice.wonderland@example.com" in index
        assert "Alice.Wonderland@Example.Com" not in index


def test_load_index_multiple_profiles():
    """Multiple profiles produce a multi-entry index."""
    with tempfile.TemporaryDirectory() as tmp:
        for name, email in [("Alice", "alice@x.com"), ("Bob", "bob@x.com")]:
            profile = {"form_data": {"email": email}, "combined": {"email": email}}
            fpath = Path(tmp) / f"profile_{name}_0.json"
            fpath.write_text(json.dumps(profile), encoding="utf-8")

        index = _load_existing_profile_index(Path(tmp))
        assert len(index) == 2
        assert index["alice@x.com"] == "profile_Alice_0.json"
        assert index["bob@x.com"] == "profile_Bob_0.json"
...
def test_load_index_skips_corrupt_json():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (Path(tmp) / "profile_Good_0.json").write_text(
            json.dumps(
                {
                    "form_data": {"email": "good@x.com"},
                    "combined": {"email": "good@x.com"},
                }
            ),
            encoding="utf-8",
        )
        ...
        index = _load_existing_profile_index(Path(tmp))
        assert len(index) == 1
        assert index["good@x.com"] == "profile_Good_0.json"
